...
async def close_investment(obj_in: Union[CharityProject, Donation]):
...
async def investment(
    ...
    if free_amount_in > free_amount_in_model:
        ...
        await close_investment(obj_model)

    elif free_amount_in == free_amount_in_model:
        await close_investment(obj_in)
        await close_investment(obj_model)
...
async def invest_process(
    ...
    objects_model = await get_not_closed_projects(model_add, session)

    for model in objects_model:
        obj_in, model = await investment(obj_in, model)
        session.add(obj_in)
        session.add(model)
